...
CAR-TER ("the app") is a JSON-driven remote-control app for iOS. This policy explains what data the app and its optional cloud service handle. The short version: **by default the app collects nothing about you and sends nothing to us** — it talks only to a server you run. One optional paid feature (Connect+) uses our relay to deliver push notifications, and the data it stores is described below.
...
We do **not** store your name, email address, contacts, or location. We do **not** sell or share this data, and we do **not** use it for advertising or analytics.
