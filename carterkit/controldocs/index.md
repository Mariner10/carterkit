...
Every control in CAR-TER is declared as a JSON object with a `type`, `id`, and grid `position`. The system supports **44 control types** across three categories:
...
### Display Controls
Gauges, progress rings, sparklines, labels, images, maps, and graphs. These visualize incoming telemetry data in real time. The [[symbol]] control is the glyph itself — SF Symbols with native rendering modes, variable layers, and looping effects, plus custom rain, snow, and sparkles, animated on the phone with no server frames.
